...
#
# Optimization:
# 1. Replaced MLP with Gated-Dconv Feed-Forward Network (GDFN) for better local detail.
# 2. Added Channel Attention (CALayer) for better feature selection.
# -----------------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------------
#  New Modules: CALayer & GatedFeedForward
# -----------------------------------------------------------------------------------
...
class CALayer(nn.Module):
    ...
    def forward(self, x):
        ...
        y = self.avg_pool(x_permuted)
        y = self.conv_du(y)
        # Permute y back to (B, 1, 1, C) and multiply
...
class GatedFeedForward(nn.Module):
    ...
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        # 3x3 Depth-wise Conv to capture local context
...
# -----------------------------------------------------------------------------------
#  Modified Blocks: SwinTransformerBlock & Cross_SwinTransformerBlock
# -----------------------------------------------------------------------------------
...
class SwinTransformerBlock(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        # [MODIFICATION] Channel Attention
        self.ca = CALayer(dim)
        
        # [MODIFICATION] GDFN replaces MLP
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = GatedFeedForward(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
    ...
    def forward(self, x, x_size):
        ...
        # [MODIFICATION] Apply Channel Attention
        x = x.view(B, H, W, C)
        x = self.ca(x)
        x = x.view(B, L, C)
        ...
        # [MODIFICATION] Pass x_size to GDFN
...
class Cross_SwinTransformerBlock(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        # [MODIFICATION] Channel Attention
        self.ca_A = CALayer(dim)
        self.ca_B = CALayer(dim)
        
        # [MODIFICATION] GDFN replaces MLP
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp_A = GatedFeedForward(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
        self.mlp_B = GatedFeedForward(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
...
# -----------------------------------------------------------------------------------
#  Rest of the Network (Layers, RSTB, Main Model)
# -----------------------------------------------------------------------------------
